module TinyClient
  class RemoteClient
    ...
    def get(path, params, id, name)
      ...
      CurbRequestor.perform_get(url, { 'Accept' => 'application/json',
                                       'Content-Type' => 'application/x-www-form-urlencoded'
      }.merge!(@config.headers), @config.connect_timeout)
    end
    ...
    def post(data, path, id, name)
      ...
      CurbRequestor.perform_post(url, { 'Accept' => 'application/json',
                                        'Content-Type' => 'application/json'
      }.merge!(@config.headers), data.to_json, @config.connect_timeout)
    end
    ...
    def put(data, path, id, name)
      url = build_url(path, id, name).build!
      CurbRequestor.perform_put(url, { 'Accept' => 'application/json',
                                       'Content-Type' => 'application/json'
      }.merge!(@config.headers), data.to_json, @config.connect_timeout)
    end
    ...
    def delete(path, id, name)
      url = build_url(path, id, name).build!
      CurbRequestor.perform_delete(url, { 'Accept' => 'application/json',
                                          'Content-Type' => 'application/x-www-form-urlencoded'
      }.merge!(@config.headers), @config.connect_timeout)
    end
